...
- 空闲时可以直接关闭。
- 正在加载映射时不销毁窗口，只提示等待加载完成。
- 正在实时恢复时不直接销毁窗口，而是先设置停止事件，按钮显示 `正在停止...`，等待恢复线程在安全检查点退出。

这样可以避免 UI 先销毁、后台线程随后继续访问窗口句柄或停止事件句柄的竞态问题。

恢复成功后，线程会同时记录源相对路径和目标相对路径到本轮内存去重集合。源路径用于避免同一轮扫描重复处理已移动文件，目标路径用于避免后续扫描把刚恢复出的明文路径误当成新的待恢复对象。
